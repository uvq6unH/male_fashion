...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validate inputs
    if (!empty($idpayment) && !empty($idtransport) && !empty($iduser) && !empty($total_money) && !empty($recipient_name) && !empty($address) && !empty($phone)) {
        $sql = "INSERT INTO orders (IDPAYMENT, IDTRANSPORT, ORDERS_DATE, IDUSER, TOTAL_MONEY, NOTES, RECIPIENT_NAME, ADDRESS, PHONE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        // Prepare and bind
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iisidssss", $idpayment, $idtransport, $orders_date, $iduser, $total_money, $notes, $recipient_name, $address, $phone);
                            ...
                            <div class="table-responsive">
                                <form method="POST" action="">
                                    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td><input class="form-control" type="number" name="idpayment" required></td>
                                                <td><input class="form-control" type="number" name="idtransport" required></td>
                                                <td><input class="form-control" type="datetime-local" name="orders_date"></td>
                                                <td><input class="form-control" type="number" name="iduser" required></td>
                                                <td><input class="form-control" type="number" step="0.01" name="total_money" required></td>
                                                <td><textarea class="form-control" name="notes"></textarea></td>
                                                <td><input class="form-control" type="text" name="recipient_name" required></td>
