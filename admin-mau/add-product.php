...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validate inputs
    if (!empty($id) && !empty($name) && !empty($idcategory) && !empty($price) && !empty($quantity) && isset($isactive)) {
        $sql = "INSERT INTO product (ID, NAME, IMAGE, IDCATEGORY, PRICE, DESCRIPTION, QUANTITY, RATING, ISACTIVE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        // Prepare and bind
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("issidsidi", $id, $name, $image, $idcategory, $price, $description, $quantity, $rating, $isactive);
