...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validate inputs
    if (!empty($id) && !empty($name) && !empty($username) && !empty($password) && !empty($email) && !empty($role) && isset($isactive)) {
        $sql = "INSERT INTO user (ID, NAME, USERNAME, PASSWORD, EMAIL, PHONE, ADDRESS, ROLE, ISACTIVE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        // Prepare and bind
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("isssssssi", $id, $name, $username, $password, $email, $phone, $address, $role, $isactive);
                                    ...
                                    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td><input class="form-control" type="text" name="role" required></td>
                                                <td>
